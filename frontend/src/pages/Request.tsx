...
//   const fetchTableContent = async () => { // Renamed to avoid confusion with initial fetchData
//     const requestId = getRequestId();
//     setLoading(true); // Start loading before fetching table content
//     try {
//       const [tableData, bulkId] = await fetchTableDataAndBulkId(requestId);
//       setBulkdataId(bulkId);
//       // Check if tableData is empty or null, and set tabledata accordingly
//       if (!tableData || tableData.length === 0) {
//         settabledata([]); // Set to empty array to ensure no old data is shown
//         // No success message needed here, as it's not an "error" but an empty state
//       } else {
//         settabledata(tableData);
//       }
//     } catch (error) { // Added error parameter for better debugging
//       // Only show a generic error if the fetch truly failed, not for empty data
//      // message.error("Failed to fetch table data.");
...
//                 ) : (
//                   <tr>
//                     <td colSpan={tablehead.length + 3} className="p-3 text-center text-gray-500">
